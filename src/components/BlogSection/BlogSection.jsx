...
If you’re looking to buy home decor online that blends comfort, quality, and timeless design, FIKA offers a curated experience for modern Indian homes. Every piece is designed to make your space feel welcoming, personal, and beautifully styled.`,
    ...
    return (
        <section className="fika-blog-section">
            <div className="fika-blog-container">
                <div className="fika-blog-header">
                    <h2 className="fika-blog-title">From Our Blog</h2>
                    <p className="fika-blog-subtitle">Discover inspiration and tips for creating your dream home</p>
                </div>

                <div className="fika-blog-grid">
                    {blogs.map((blog) => (
                        <article key={blog.id} className="fika-blog-card">
                            <div className="fika-blog-image-wrapper">
                                <img
                                    src={blog.image}
                                    alt={blog.title}
                                    className="fika-blog-image"
                                    onError={(e) => {
                                        e.target.src = '/placeholder-blog.jpg';
                                    }}
                                />
                                <div className="fika-blog-category-badge">{blog.category}</div>
                            </div>

                            <div className="fika-blog-content">
                                <div className="fika-blog-meta">
                                    <span className="fika-blog-date">{blog.date}</span>
                                </div>

                                <h3 className="fika-blog-card-title">{blog.title}</h3>
                                <p className="fika-blog-excerpt">{blog.excerpt}</p>

                                <Link to={`/blog/${blog.id}`} className="fika-blog-read-more">
                                    Read More
                                    <svg
                                        className="fika-blog-arrow-icon"
                                        width="16"
                                        height="16"
                                        viewBox="0 0 16 16"
                                        fill="none"
