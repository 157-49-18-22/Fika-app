...
const Analytics = () => {
  ...
  const fetchProductsData = async () => {
    try {
      ...
      // Query wish genie products
      const wishGenieQuery = query(
        collection(db, 'wish_genie'),
        limit(100)
      );
      
      const [productsSnapshot, wishGenieSnapshot] = await Promise.all([
        getDocs(productsQuery),
        getDocs(wishGenieQuery)
      ]);
